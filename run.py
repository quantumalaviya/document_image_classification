...
from transformers import AutoTokenizer, LayoutLMv2FeatureExtractor, LayoutLMv2Processor, AutoModelForSequenceClassification
...
method_columns = {
    ...
    "microsoft/layoutlm-base-uncased": ['input_ids', 'bbox', 'attention_mask', 'token_type_ids', 'labels'],
    "microsoft/layoutlmv2-base-uncased": ['image', 'input_ids', 'bbox', 'attention_mask', 'token_type_ids', 'labels'],
}
...
if __name__ == "__main__":
    ...
    if "v2" in method:
        ...
        train_ds = train_ds.map(lambda example: encode_example_v2(example, processor), features=Features(features), num_proc=os.cpu_count())
        val_ds = val_ds.map(lambda example: encode_example_v2(example, processor), features=Features(features), num_proc=os.cpu_count())
    else:
        # TODO: DRY
        train_ds = train_ds.map(lambda example: encode_example(example, tokenizer), features=Features(features), num_proc=os.cpu_count())
        val_ds = val_ds.map(lambda example: encode_example(example, tokenizer), features=Features(features), num_proc=os.cpu_count())
